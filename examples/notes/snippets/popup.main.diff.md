# `src/entrypoints/popup/PopupApp.tsx`

Replace the body with the demo panel:
...
Re-add the mouseup-driven save-selection button alongside the toast listener:
...
function ContentApp() {
  ...
  useEffect(() => {
    const handleMouseUp = () => setSelectedText(getSelectedText());

    const contentMessageListener = createContentMessageListener((message) => {
      setToastMessage(message);
      setToastVisible(true);
      globalThis.setTimeout(() => setToastVisible(false), 1800);
    });
